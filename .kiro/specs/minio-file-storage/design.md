...
将AI知识库应用的文件存储从本地文件系统迁移到S3兼容的对象存储。保持现有API不变，替换底层存储实现。
...
- 支持现有的分块上传功能
- 提供数据迁移工具
- 支持多种S3兼容服务（MinIO、AWS S3、阿里云OSS、腾讯云COS等）

## 架构

简单的替换策略：
1. 保持现有的DocumentService接口不变
2. 将文件存储从本地文件系统改为S3兼容的对象存储
3. 使用S3的multipart upload替代现有的分块上传
...
S3客户端 (新增)
    ↓  
S3兼容存储服务 (MinIO/AWS S3/OSS/COS等)
```
...
type Document struct {
    ...
    FilePath     string    `json:"file_path"`     // 改为存储S3对象键
...
使用现有的错误处理机制，S3错误直接返回给调用者。对于网络错误，使用简单的重试：
...
**属性 1: 文件上传往返一致性**
*对于任何* 成功上传的文件，从S3兼容存储下载的文件内容应该与原始上传的文件内容完全相同
**验证: 需求 2.3, 3.1**
...
**属性 3: 文件去重正确性**
*对于任何* 具有相同哈希值的文件，系统应该在S3兼容存储中仅存储一个副本
**验证: 需求 2.5, 8.1, 8.2, 8.5**

**属性 4: API兼容性**
*对于任何* 现有的API调用，S3兼容存储实现应该返回与本地存储实现相同格式的响应
**验证: 需求 2.1**
